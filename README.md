...
| 0.15.1 | **The note's four extra fields are required, and sit on the screen rather than behind a picker.** Service Description, Additional Remarks, City and State Where Service Was Provided are how this organization fills out every note it issues, so Create now asks for all four directly once **Issue Invoice** is anything but *Don't issue*, and refuses the creation if one is left empty. ⚠️ **Breaking, on a field published hours earlier:** 0.15.0's **Invoice Fields** — the collapsed *Add Field* picker holding the same four — is gone, and a workflow built against it in that short window needs the four re-entered under their new names |
