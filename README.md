...
**Picking any answer other than *Don't issue* asks four more fields, all of them required:**
Service Description, Additional Remarks, City Where Service Was Provided and State Where Service
Was Provided. They sit directly on the screen rather than behind an *Add Field* picker, and the
node refuses the creation if one is left empty — this organization fills all four on every note
it issues, so the screen asks for them up front instead of letting an incomplete one through.
...
Two facts about the automations, measured on a production organization on 2026-08-02:

- **The created item does not echo the automation keys.** The get-by-id this API reads a
  schedule through answers no `autoGenerateCollectionType` and no `autoGenerateNFSeType` — those
  live in the **Get Many** DTO only. To see what a schedule has programmed, list with a filter
  on `scheduleId`.
- ⚠️ **An update disarms what the creation programmed.** Any `PUT` on a schedule wipes both
  `autoGenerate*` fields unless the body carries the automation again, in the exact spelling of
  the creation — and no client can carry it, because no read answers the days or the profile IDs
  back. So an **Update** through this node, or through anything else that speaks this API,
  switches a pending boleto or NFS-e automation off. Finish the updates first and create with
  the automation last, or recreate the schedule.
...
> 🔴 **Both delivery types send the charge — the choice is which way out, not whether.** Nibo has a financial-management module and an accountant one. *By E-Mail* sends the boleto to the payer from the financial side; *Through the Accountant* hands the charge to the accountant module, where it reaches the payer in the client portal along with the rest of that module's traffic. Nothing on that screen keeps a charge in. The effect is readable afterwards, in `accountantIntegrationStatus`: *Não definida* for the first, *Habilitada* for the second — which records the **request**, not its outcome, and can later become *"Não foi possível criar o item de conferência no Nibo Obrigações"*. Routing through the accountant only makes sense for an organization whose Nibo is integrated with that module.
>
> **0.13.0 got this wrong**: it labelled the two *Send It* and *Hold for Review* and stated what each did, from a claim in this project's own notes that nobody had verified. There is no hold. Corrected in 0.13.1.
...
**It asks for six things, all required: the schedule, the service profile, the accrual date, the service description, and where the service was provided — city and state.** ⚠️ **Required since 0.15.1**, and sitting directly on the screen rather than behind an *Additional Fields* picker: this organization fills all three of the last group on every note it issues, so the node asks for them up front and refuses the issuing if one is left empty. Neither the amount nor the taker is on the screen: both come from the schedule. The API does want the taker as a key of its own, and until 0.14.0 the node asked for it; since **0.14.1** it reads it off the schedule, because a receivable already names its contact and asking twice was only a chance for the two to disagree. Whoever needs a different taker changes the contact on the schedule.

That reading is also the guard this operation needed: **this API validates what is written and never what it is written onto**, so the schedule is read before anything is sent. A wrong schedule ID stops at the node, not at a city hall.
...
**The status codes**, measured — and they are **two mirrored pipelines**:

| On the way in | On the way out |
|---|---|
| **1** queued for processing *(transient)* | **-2** queued for cancellation *(transient)* |
| **2** waiting for authorization *(transient)* | **-3** cancellation in progress *(transient)* |
| **3** authorized | **-4** cancelled |
| **-1** denied | |

Only **3**, **-1** and **-4** are places a note stays. Only at **3** does it gain its `number`, its `verificationCode`, its PDF and its XML — those four are `undefined` until then, and a denied note never gets them. This API has no route that lists the whole set, so for a state that is not here, write the condition in **Filter (OData)**.

> 🔴 **`-2` and `-3` were found in the acceptance of 0.14.0, not before it.** The plan behind this version had them down as never seen, and the first cancellation run through the node walked straight through both. Practical consequence: **the read-back right after Cancel shows `-2`, not `-4`** — the cancellation has its own queue and lands on `-4` seconds later. The node hands the record back saying so in `_niboCancellationPending`, and never calls it a failure.
...
**For a contact** that is the **Name**, and nothing else. **Document Number** and **Document Type** are on the same screen but are no longer demanded — the API stores a contact that has only a name, which is what a lead or a form usually is. Three details of this API are worth knowing before you fill the form in:

- **A document left empty is no document at all.** The node does not send an empty one: the number and the type leave the payload together, so the contact is created without them and nothing is there to be refused. Such a contact reads back with `document` as an **empty object** — measured on all four collections — so a workflow reading `document.number` gets nothing rather than an empty string. Fill the number in later with **Update**, which takes it like any other field.
