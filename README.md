...
> 🔴 **Issuing a charge can deliver it to the payer, and nothing on this screen stops that.** Charges in this API reach a `deliveryStatus` of *Delivered* and then *Viewed* on their own. What triggers it could not be established: the API's `deliveryType` field is undocumented by Nibo, never comes back on the record, and the two values this project tried produced no visible difference. So the node offers the two as the bare numbers they are, sends the field only when you pick one, and promises nothing about either. **0.13.0 got this wrong** — it labelled them *Send It* and *Hold for Review* and stated what each did, from a claim in this project's own notes that nobody had verified. Corrected in 0.13.1.
...
| 0.13.1 | Fix, and the fault was in the labels rather than the code: **Delivery** offered *Send It* against *Hold for Review* and stated on screen what each one did. Neither was ever measured — both came from a claim in this project's own notes, which Nibo documents nowhere, and the one measurement taken had found the two values indistinguishable. Saying "nothing is sent" about an unverified setting is the dangerous half of that pair, because it invites issuing a charge in the belief that it stays put. The field is now **Delivery Type**, offering the two raw values under no promise at all, and **it is not sent unless you pick one**. The notice says what is measured instead: a charge does reach *Delivered* and then *Viewed* on its own, and nothing on that screen was shown to prevent it |
