...
**There is no total to type.** The amount of a schedule is the sum of its category lines, and each line can carry a **Description** of its own. Type the line amounts **positive on both kinds** — that is what the API takes, and it signs them itself.

> ⚠️ **Splitting across several categories is not available in every organization.** Measured: a creation with two lines can be refused with *"Utilize apenas uma categoria"*. Nothing in the API says which organizations allow it, so the field offers as many lines as you like and the node explains that refusal when it arrives — leave one line, or make one schedule per category.
